...
public class DangNhapJDialog extends javax.swing.JDialog {
    ...
    private void btnDangNhapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDangNhapActionPerformed
        ...
        try {
            ...
            this.SLDangNhapSai = Auth.nguoiDungHienTai.getSLDangNhapSai();
            if (Auth.nguoiDungHienTai == null) {
                loi.append("Bạn đã nhập Sai tài khoản!");
                MessegerHelper.errorMesseger(loi, this);
                return;
            } else if (Auth.nguoiDungHienTai.isTrangThai() == false) {
                MessegerHelper.errorMesseger(new StringBuilder("Tài khoản của bạn đã bị khóa!"), this);
                return;
            } else if (!matKhau.equals(Auth.nguoiDungHienTai.getMatKhau())) {
                this.SLDangNhapSai++;
                Auth.nguoiDungHienTai.setSLDangNhapSai(SLDangNhapSai);
                dao.update(Auth.nguoiDungHienTai);
                if (SLDangNhapSai >= 5) {
                    MessegerHelper.errorMesseger(new StringBuilder("Bạn đã nhập sai quá số lần quy định,"
                            + "\nTài khoản của bạn đã bị khóa!"), this);
                    Auth.nguoiDungHienTai.setTrangThai(false);
                    dao.update(Auth.nguoiDungHienTai);
                    return;
                }
                loi.append("Bạn đã nhập Sai mật khẩu " + SLDangNhapSai + " Lần\n"
                        + "Bạn chỉ được phép sai tối đa 5 lần!");
                MessegerHelper.errorMesseger(loi, this);
                return;
            }
            Auth.nguoiDungHienTai.setSLDangNhapSai(0);
            dao.update(Auth.nguoiDungHienTai);
            this.dispose();
        } catch (Exception ex) {
